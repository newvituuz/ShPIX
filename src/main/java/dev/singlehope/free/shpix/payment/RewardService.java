    ...
    private void announce(final Player player, final Product product, final Order order, final ActionType type) {
        ...
        if (!resolved.title().isBlank() || !resolved.subtitle().isBlank()) {
            player.showTitle(Title.title(
                    Text.color(Text.apply(resolved.title(), values)),
                    Text.color(Text.apply(resolved.subtitle(), values)),
                    TIMES));
        }
        if (!resolved.actionBar().isBlank()) {
            player.sendActionBar(Text.color(Text.apply(resolved.actionBar(), values)));
        }
        if (!resolved.message().isBlank()) {
            for (final String line : Text.apply(resolved.message(), values).split("\n", -1)) {
                final Component component = Text.color(line);
