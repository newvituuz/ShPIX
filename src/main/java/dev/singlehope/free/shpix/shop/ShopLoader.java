...
import dev.singlehope.free.shpix.compat.Sounds;
...
public final class ShopLoader {
    ...
    private List<Category> loadCategories() {
        ...
        for (final String key : section.getKeys(false)) {
            ...
            categories.add(new Category(
                    ...
                    material(entry.getString("icon.material"), "categoria " + key),
    ...
    private List<Product> loadProducts(final List<Category> categories, final PluginConfig config) {
        ...
        final File[] existing = directory.listFiles((dir, name) -> name.toLowerCase(Locale.ROOT).endsWith(".yml"));
        if (existing == null || existing.length == 0) {
            try {
                this.plugin.saveResource("products/exemplo.yml", false);
            } catch (IllegalArgumentException ignored) {
                // recurso ausente no jar
            }
        }
    ...
    private Rewards readRewards(final YamlConfiguration yaml, final String productId) {
        ...
        if (section != null) {
            for (final String key : section.getKeys(false)) {
                ...
                items.add(new RewardItem(key, data, amount));
            }
        }
    ...
    private String material(final String raw, final String context) {
        if (raw == null || raw.isBlank() || !Materials.isKnown(raw)) {
    ...
    private Sound sound(final String raw, final String productId) {
        ...
        return Sounds.resolve(raw).orElseGet(() -> {
