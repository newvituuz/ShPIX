...
public final class LoadingMenu extends Menu {
    ...
    public LoadingMenu(final Plugin plugin, final Player player, final Component title) {
        super(title, 3);
        this.plugin = plugin;
        this.player = player;
    }
    ...
    private void drawRing(final int headIndex, final int tailIndex) {
        final ItemStack idle = new ItemBuilder(Material.GRAY_STAINED_GLASS_PANE).name("&7 ").build();
        final ItemStack head = new ItemBuilder(Material.LIME_STAINED_GLASS_PANE).name("&a&lGERANDO QR CODE...").build();
        final ItemStack tail = new ItemBuilder(Material.GREEN_STAINED_GLASS_PANE).name("&a&lGERANDO QR CODE...").build();
