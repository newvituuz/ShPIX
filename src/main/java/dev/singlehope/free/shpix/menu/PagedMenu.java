...
public abstract class PagedMenu extends Menu {
    ...
    protected PagedMenu(final String title) {
        super(title, 6);
    }
    ...
    protected void build(final Player viewer) {
        ...
        if (this.page > 0) {
            set(PREVIOUS_SLOT, new ItemBuilder("ARROW").name("&aPágina anterior").build(), event -> {
                this.page--;
                refresh(viewer);
            });
        }
        if (this.page < pages - 1) {
            set(NEXT_SLOT, new ItemBuilder("ARROW").name("&aPróxima página").build(), event -> {
                this.page++;
                refresh(viewer);
            });
        }
        set(CLOSE_SLOT, new ItemBuilder("BARRIER").name("&cFechar").build(),
