...
public final class CheckoutMenu extends Menu {
    ...
    public CheckoutMenu(final ShPixPlugin plugin, final Product product, final Coupon coupon) {
        super("&8Pagamento - " + product.name(), 3);
        this.plugin = plugin;
    ...
    protected void build(final Player viewer) {
        ...
        if (available.isEmpty()) {
            set(13, new ItemBuilder("BARRIER").name("&cNenhum método de pagamento disponível.").build());
        }
        ...
        set(20, new ItemBuilder("NAME_TAG")
                ...
                .lore(List.of(
                        ...
                        this.coupon == null
                                ? "&eNenhum cupom ativo."
                                : "&aCupom ativo: &f" + this.coupon.name()
                                        + " &7(" + this.coupon.percent().toPlainString() + "%)"))
                .build(),
                event -> requestCoupon(viewer));

        set(24, new ItemBuilder("ARROW").name("&cVoltar").build(),
    ...
    private ItemStack gatewayIcon(final GatewayType type, final PriceQuote quote) {
        ...
        final ItemBuilder builder = new ItemBuilder("PLAYER_HEAD")
    ...
    private void submit(final Player viewer, final GatewayType gateway) {
        ...
        final LoadingMenu loading = new LoadingMenu(this.plugin, viewer, "&8Gerando QR Code...");
        loading.start();
    ...
    private boolean hasRoomForRewards(final Player viewer) {
        ...
        for (int slot = 0; slot < 36; slot++) {
            final ItemStack stack = viewer.getInventory().getItem(slot);
