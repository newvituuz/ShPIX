...
public final class IndexMenu extends Menu {
    ...
    public IndexMenu(final ShPixPlugin plugin) {
        super(Text.color("&8Loja Virtual"), 3);
        this.plugin = plugin;
    }
    ...
    protected void build(final Player viewer) {
        ...
        set(11, new ItemBuilder(Material.PLAYER_HEAD)
        ...
        set(13, new ItemBuilder(Material.CHEST)
                .name("&aCategorias")
                .legacyLore(List.of("&7Navegue pelos produtos disponíveis.", "", "&eClique para abrir."))
                .build(),
                event -> new CategoryMenu(this.plugin).open(this.plugin, viewer));

        set(15, new ItemBuilder(Material.BOOK)
                .name("&aMeus pedidos")
                .legacyLore(List.of("&7Veja os seus últimos pedidos.", "", "&eClique para abrir."))
                .build(),
                event -> openOrders(viewer));

        set(22, new ItemBuilder(Material.BARRIER).name("&cFechar").build(),
