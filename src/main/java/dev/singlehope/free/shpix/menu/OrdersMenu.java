...
public final class OrdersMenu extends PagedMenu {
    ...
    public OrdersMenu(final ShPixPlugin plugin, final List<Order> orders) {
        super(Text.color("&8Loja Virtual - Meus pedidos"));
        this.plugin = plugin;
    ...
    protected List<Entry> entries(final Player viewer) {
        ...
        for (final Order order : this.orders) {
            ...
            final ItemStack icon = new ItemBuilder(iconFor(order.status()))
                    ...
                    .build();
            entries.add(new Entry(icon, event -> {
            }));
        }
        return entries;
    }

    private static Material iconFor(final OrderStatus status) {
        return switch (status) {
            case DELIVERED, PAID -> Material.EMERALD;
            case WAITING -> Material.CLOCK;
            case REFUNDED -> Material.GOLD_INGOT;
            case EXPIRED, CANCELLED -> Material.REDSTONE;
        };
    }
    ...
    protected ItemStack emptyIcon() {
        return new ItemBuilder(Material.BARRIER).name("&cVocê ainda não fez nenhum pedido.").build();
    }
}
